...
class Config:
    ...
    APP_VERSION = "Data Intake 2.0"
    APP_BUILD_DATE = "05/06/2026"
...
class DJISequenceThread(QThread):
    ...
    def run(self):
        for cmd in self._cmds:
            print(f"[DJI] starting: {cmd[0]}")
            result = subprocess.run(cmd)
            print(f"[DJI] finished: {cmd[0]} (exit {result.returncode})")
            if result.returncode != 0:
                print(f"[DJI] aborting sequence — non-zero exit from {cmd[0]}")
                break
...
class ProcessingWorker(QThread):
    ...
    def _process_sensor_specific(self, sensor_folder: str, date_folder: str):
        ...
        if self.sensor_choice in ("L2", "L3"):
            ...
            self._process_standard_sensor(base_folder)
    ...
    def _process_standard_sensor(self, base_folder: str):
...
class EPSGLookupDialog(QDialog):
    ...
    def __init__(self, parent=None, mode="h"):
        super().__init__(parent)
        self._mode = mode  # "h" → insert EPSG col, "v" → insert Vertical col
        self.selected_code = None
        self.setWindowTitle("EPSG Lookup — Horizontal" if mode == "h" else "EPSG Lookup — Vertical")
        self.resize(520, 460)
    ...
    def _accept_selection(self):
        ...
        if row < 0:
            return
        col = 2 if self._mode == "h" else 3
        self.selected_code = self._table.item(row, col).text()
        self.accept()
...
class DataIntakeUI(QMainWindow):
    ...
    def _add_dji_settings_section(self):
        ...
        epsg_h_search.clicked.connect(lambda: self._open_epsg_lookup(self.epsg_h_input, "h"))
    ...
    def _open_epsg_lookup(self, target_field, mode):
        """Open EPSG lookup dialog and write selected code into target_field."""
        dlg = EPSGLookupDialog(self, mode=mode)
        if dlg.exec_() == QDialog.Accepted and dlg.selected_code:
            target_field.setText(dlg.selected_code)
    ...
    def _handle_complete(self, client, project, date, sensor_path, image, files, date_path):
        ...
        is_lidar     = any(s in sensor_path for s in ("L2", "L3"))
        ...
        dji_cmds = []
        if self.dji_ppk_toggle and self.dji_ppk_toggle.isChecked() and is_lidar:
            cmd = [
        ...
        if self.dji_terra_toggle and self.dji_terra_toggle.isChecked() and is_lidar:
            cmd = [
