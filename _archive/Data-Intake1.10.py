...
class ProcessingWorker(QThread):
    ...
    def cleanup_ppk_folder(self, ppk_folder_path, date_folder_path):
        ...
        # Find RINEX file and run L2_rename/L3_rename and RTB processing for L2/L3 sensors
        ...
        if rinex_file_path:
            if self.sensor_choice == "L2":
                ...
                print(f"Calling L3_rename with file: {rinex_file_path}")
                self.L3_rename(rinex_file_path, os.path.join(date_folder_path, "L3"))
                # Skip RTB processing for L3 sensor (processed via DJI Terra instead)
                self.status_update.emit("L3 sensor detected - skipping RTB processing (use DJI Terra)")
                print("Skipping RTB for L3 sensor - process in DJI Terra")
        else:
            print(f"No RINEX .??o file found - skipping L2_rename/L3_rename and RTB.")
    ...
    def batch_convert_to_rinex(self, folder_path):
        ...
        for file_name in t02_t04_files:
            try:
                ...
                command = [convert_to_rinex_exe, file_path]
        ...
        try:
            ...
            for entry in entries:
                ...
                if self.sensor_choice in ("L2", "L3"):
                    device_arg = "dji_l2"  # REDToolBox uses dji_l2 for both L2 and L3
                else:
                    device_arg = "dji"
                cmd = [
                    cli, "mapping",
    ...
    def L3_rename(self, file_path, root_dir):
        """Rename RINEX files for L3 processing"""
        file_to_copy_name = os.path.basename(file_path)
        for subdir, dirs, files in os.walk(root_dir):
            rtk_file = None
            for filename in files:
            ...
            if rtk_file:
                ...
                print(f"No .rtk file found in folder '{subdir}'. Skipping folder.")
        print("L3_rename script completed!")
...
class DataIntakeUI(QMainWindow):
    ...
    def start_create_subfolders_thread(self):
        ...
        if not sensor_choice:
            error_msg = "Could not determine the sensor from image metadata."
            if exif_model:
                error_msg += f"\n\nEXIF Model found: '{exif_model}'\nThis model is not in the supported list."
                error_msg += f"\n\nSupported models: {list(EXIF_MODEL_TO_SENSOR.keys())}"
            else:
                error_msg += "\n\nNo EXIF Model data found in images."
            if image_path_used:
                error_msg += f"\n\nImage checked: {image_path_used}"
            self.show_message(
