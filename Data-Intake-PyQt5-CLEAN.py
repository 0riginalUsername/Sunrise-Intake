...
class ProcessingWorker(QThread):
    ...
    def __init__(self, selected_folder, data_source_folders, base_data_path, client, project, sensor_choice):
        super().__init__()
        ...
        try:
            ...
            if self.sensor_choice == "L2":
                base_data_targets = [
                    ...
                    os.path.join(self.selected_folder, self.client, self.project, DATE_CURR, "PPK", "BaseData")
                ]
                self.copy_base_data(self.base_data_path, base_data_targets)
    ...
    def copy_base_data(self, base_data_path, base_data_targets):
        """Copy base data file to target locations"""
        if not os.path.isfile(base_data_path):
            return
            
        file_name = os.path.basename(base_data_path)
        for target_folder in base_data_targets:
            ...
            try:
                shutil.copy2(base_data_path, dest_file)
            except Exception as e:
    ...
    def cleanup_ppk_folder(self, ppk_folder_path, date_folder_path):
        ...
        # Run RINEX conversions
        self.status_update.emit("Converting to RINEX format...")
        self.batch_convert_to_rinex(os.path.join(date_folder_path, "BaseData"))
        self.batch_convert_to_rinex(os.path.join(ppk_folder_path, "BaseData"))
        
        # Find RINEX file and run L2_rename and RTB processing for L2 sensors
        rinex_folder = os.path.join(ppk_folder_path, "BaseData")
        rinex_file_path = None
        for root_dir, _, files in os.walk(rinex_folder):
            for file in files:
                if file.lower().endswith("o"):
    ...
    def batch_convert_to_rinex(self, folder_path):
        ...
        if not os.path.isdir(folder_path):
            ...
            return
        t02_t04_files = [f for f in os.listdir(folder_path) if f.endswith((".T02", ".T04", ".t02", ".t04"))]
    ...
    def run_rtb(self, image_dir, base_dir, ppk_folder):
        try:
            ...
            bfile = self.find_latest(os.path.join(base_dir, "*.*o"))
            nfile = self.find_latest(os.path.join(base_dir, "*.*n"))
            gfile = self.find_latest(os.path.join(base_dir, "*.*g"))
            
            print(f"Found files: bfile={bfile}, nfile={nfile}, gfile={gfile}")
            if not all((bfile, nfile, gfile)):
                print("ERROR: Could not find one of .o, .n or .g files in base directory.")
            ...
            for entry in entries:
                ...
                missing = []
                for path in [Lfile, Rfile, bfile, nfile, gfile, subdir_path, output_dir]:
            ...
            print(f"Critical error in run_rtb: {e}")
...
class DataIntakeUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.base_file_drop = QLabel("Drop Base Data file here or click to select")
        ...
        self.basefile_clear_btn.clicked.connect(lambda: self.label5.setText("Base data file not selected"))
    ...
    def choose_base_file_event(self, event):
        file, _ = QFileDialog.getOpenFileName(self, "Select Base Data file", "", "T02/T04 Files (*.T02 *.T04);;All Files (*)")
        if not file:
            return
    ...
    def on_base_file_drop(self, event):
        for url in event.mimeData().urls():
            ...
            if os.path.isfile(path) and path.lower().endswith(('.t02', '.t04')):
                self.set_base_file(path)
                break

    def set_base_file(self, file):
        self.base_data_path = file
        self.label5.setText(f"Selected file: {os.path.basename(file)}")
        self.set_names()
    ...
    def start_create_subfolders_thread(self):
        ...
        if not self.base_data_path:
            QMessageBox.warning(self, "No Base File", "Please select a base data file.")
            return
