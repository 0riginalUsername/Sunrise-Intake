...
class Config:
    ...
    APP_VERSION = "Data Intake 1.14"
    APP_BUILD_DATE = "03/30/2026"
...
class RinexProcessor:
    ...
    def rename_for_sensor(rinex_file_path: str, sensor_folder: str) -> None:
        """
        Rename RINEX files for L2/L3 processing.
        Unified method replacing L2_rename and L3_rename (they were identical).
        """
...
# =============================================================================
# RTB PROCESSOR MODULE - REDToolBox CLI integration
# =============================================================================

class RTBProcessor:
    """Handles REDToolBox CLI processing."""
    
    @staticmethod
    def get_device_arg(sensor: str) -> str:
        """Get the appropriate device argument for RTB CLI."""
        if sensor in ("L2", "L3"):
            return "dji_l2"  # REDToolBox uses dji_l2 for both L2 and L3
        return "dji"
    
    @staticmethod
    def resolve_atx_file(antenna_name: Optional[str]) -> Optional[str]:
        """Resolve or download ATX file for antenna."""
        if not antenna_name:
            return None
        
        try:
            os.makedirs(Config.ATX_REPO_DIR, exist_ok=True)
            atx_path = os.path.join(Config.ATX_REPO_DIR, antenna_name)
            
            if os.path.isfile(atx_path):
                print(f"Using existing ATX: {atx_path}")
                return atx_path
            
            url = f"https://geodesy.noaa.gov/ANTCAL/LoadFile?file={antenna_name}"
            print(f"Downloading ATX from {url}")
            urllib.request.urlretrieve(url, atx_path)
            
            if os.path.isfile(atx_path):
                print(f"Downloaded ATX to {atx_path}")
                return atx_path
        except Exception as e:
            print(f"Failed to resolve ATX: {e}")
        
        return None
    
    @staticmethod
    def build_command(
        sensor: str, subdir: str, output_dir: str,
        log_file: str, rover_file: str, base_file: str,
        nav_file: str, atx_file: Optional[str], gnav_file: Optional[str],
        correction_type: str = "ppk",
    ) -> List[str]:
        """Build the RTB CLI command."""
        ct = (correction_type or "ppk").strip().lower()
        if ct not in ("ppk", "rtb"):
            ct = "ppk"
        cmd = [
            Config.RTB_CLI_EXE, "mapping",
            "--device", RTBProcessor.get_device_arg(sensor),
            "--correction-type", ct,
            "--output-format", "exif",
            "--output-format", "pdf",
        ...
        if gnav_file:
            cmd.extend(["--gnav-file", gnav_file])
        return cmd
...
class ProcessingWorker(QThread):
    ...
    def __init__(
        ...
        ct = (rtb_correction_type or "ppk").strip().lower()
        self.rtb_correction_type = ct if ct in ("ppk", "skip") else "ppk"
    ...
    def _process_sensor_specific(self, sensor_folder: str, date_folder: str):
        ...
        if self.sensor_choice in ("L2", "L3"):
            self._process_lidar_sensor(sensor_folder, date_folder, base_folder)
    ...
    def _process_lidar_sensor(self, sensor_folder: str, date_folder: str, base_folder: str):
        """Process L2/L3 LiDAR sensors."""
        self._copy_base_data([base_folder])
        
        ppk_folder = os.path.join(date_folder, "PPK")
        
        # Copy sensor data to PPK
        self._copy_to_ppk(sensor_folder, ppk_folder)
        
        # Cleanup and process
        self._cleanup_ppk_folder(ppk_folder, date_folder)
    ...
    def _process_standard_sensor(self, sensor_folder: str, base_folder: str):
        ...
        if self.rtb_correction_type == "ppk":
            self.status_update.emit("Running RTB processing...")
            self._run_rtb(sensor_folder, base_folder, None)
        else:
            self.status_update.emit("No PPK selected: skipping RTB processing.")
    ...
    def _cleanup_ppk_folder(self, ppk_folder: str, date_folder: str):
        ...
        if rinex_file:
            ...
            if self.rtb_correction_type == "ppk":
                self.status_update.emit(f"Running RTB processing for {self.sensor_choice}...")
                self._run_rtb(ppk_folder, base_data_ppk, ppk_folder)
            else:
                self.status_update.emit("No PPK selected: skipping RTB processing.")
    ...
    def _run_rtb(self, image_dir: str, base_dir: str, ppk_folder: Optional[str]):
        """Run REDToolBox processing."""
        try:
            logger.info("REDToolBox correction-type: %s", self.rtb_correction_type)
            # Find required files
            bfile = FileOperations.find_files_with_year_suffix(base_dir, {".o", ".obs"})
            nav_candidates = RinexProcessor.collect_nav_files(base_dir)
            
            merge_needed = (len(nav_candidates) > 1 and 
                          (len(self.base_data_paths) > 1 or self.base_data_is_rinex))
            nfile = (RinexProcessor.merge_nav_files(nav_candidates, base_dir) 
                    if merge_needed else (nav_candidates[0] if nav_candidates else None))
            
            gfile = FileOperations.find_files_with_year_suffix(base_dir, {".g"})
            ant_atx = RTBProcessor.resolve_atx_file(
                RinexProcessor.extract_antenna_filename(bfile)
            )
            
            if not all((bfile, nfile)):
                print("ERROR: Missing required base/nav files")
                return
            ...
            for step, entry in enumerate(entries):
                ...
                if ppk_folder:
                    ...
                    rover_file = FileOperations.find_files_by_glob(os.path.join(subdir, "*.RTK"))
                else:
                    output_dir = os.path.join(image_dir, "RTB")
                    rover_file = FileOperations.find_files_by_glob(os.path.join(subdir, "*.obs"))
                
                os.makedirs(output_dir, exist_ok=True)
                log_file = FileOperations.find_files_by_glob(os.path.join(subdir, "*Timestamp.MRK"))
                
                # Validate files
                if not all((log_file, rover_file)):
                    print(f"Skipping {entry.name}: missing files")
                    self.file_copy_progress.emit(step + 1, len(entries), f"Skipped {entry.name}")
                    continue
                
                # Run RTB
                cmd = RTBProcessor.build_command(
                    self.sensor_choice, subdir, output_dir,
                    log_file, rover_file, bfile, nfile, ant_atx, gfile,
                    correction_type=self.rtb_correction_type,
                )
                
                self.status_update.emit(f"Processing: {entry.name} ({step + 1}/{len(entries)})")
                
                try:
                    run_subprocess(cmd, check=True)
                    self.file_copy_progress.emit(step + 1, len(entries), f"RTB processed: {entry.name}")
                except subprocess.CalledProcessError as e:
                    print(f"RTB error for {entry.name}: {e}")
                    self.file_copy_progress.emit(step + 1, len(entries), f"RTB error: {entry.name}")
            
            self.status_update.emit("RTB processing completed.")
            self._copy_rtb_images_to_exif_c(image_dir, ppk_folder)
            self._open_pdfs(ppk_folder or image_dir)
            
        except Exception as e:
            logger.exception("Critical error in RTB processing")
            self.error_occurred.emit(f"RTB error: {e}")
    
    def _copy_rtb_images_to_exif_c(self, image_dir: str, ppk_folder: Optional[str]):
        """Copy JPGs from RTB output to exif_C folder."""
        exif_c = os.path.join(image_dir, "exif_C")
        os.makedirs(exif_c, exist_ok=True)
        
        rtb_dir = os.path.join(ppk_folder or image_dir, "RTB")
        if not os.path.exists(rtb_dir):
            return
        
        jpg_files = [
            (root, f) for root, _, files in os.walk(rtb_dir)
            for f in files if f.lower().endswith('.jpg')
        ]
        
        for i, (root, file) in enumerate(jpg_files):
            if self.should_stop:
                break
            src = os.path.join(root, file)
            FileOperations.copy_file_with_dedup(src, exif_c)
            self.file_copy_progress.emit(i + 1, len(jpg_files), f"exif_C ({i + 1}/{len(jpg_files)})")
    
    def _open_pdfs(self, base_dir: str):
        """Open generated PDF reports."""
        rtb_dir = os.path.join(base_dir, "RTB")
        if not os.path.exists(rtb_dir):
            return
...
class DataIntakeUI(QMainWindow):
    ...
    def __init__(self):
        ...
        self.log_file_path = None
        self.rtb_correction_type = "ppk"  # from main-window checkboxes; passed to ProcessingWorker
        ...
        self.start_btn = None
        self.cb_run_ppk = None
        self.cb_no_ppk_rtb = None
    ...
    def _init_ui(self):
        ...
        self._add_rtb_correction_section()
    ...
    def _add_folder_selection(self):
        ...
        clear_btn.clicked.connect(lambda: self.folder_display.setText(""))
    ...
    def _add_rtb_correction_section(self):
        """REDToolBox PPK vs RTB — main window (not Exif popup)."""
        cap = QLabel("REDToolBox correction (all flights)")
        cap.setFont(QFont("Segoe UI", 12, QFont.Bold))
        cap.setStyleSheet(Styles.LABEL_TRANSPARENT)
        self.scroll_layout.addWidget(cap)
        row = QHBoxLayout()
        row.setAlignment(Qt.AlignHCenter)
        self.cb_run_ppk = QCheckBox("Run PPK")
        self.cb_no_ppk_rtb = QCheckBox("No PPK — Skip RTB")
        self.cb_run_ppk.setFont(QFont("Segoe UI", 11))
        self.cb_no_ppk_rtb.setFont(QFont("Segoe UI", 11))
        self.cb_run_ppk.setStyleSheet(Styles.CHECKBOX_MAIN)
        self.cb_no_ppk_rtb.setStyleSheet(Styles.CHECKBOX_MAIN)
        self.cb_run_ppk.stateChanged.connect(self._on_cb_run_ppk)
        self.cb_no_ppk_rtb.stateChanged.connect(self._on_cb_no_ppk_rtb)
        row.addWidget(self.cb_run_ppk)
        row.addWidget(self.cb_no_ppk_rtb)
        self.scroll_layout.addLayout(row)
        self._set_rtb_correction_checkboxes("ppk")
    
    def _set_rtb_correction_checkboxes(self, mode: str) -> None:
        ct = "skip" if (mode or "").strip().lower() == "skip" else "ppk"
        if not self.cb_run_ppk or not self.cb_no_ppk_rtb:
            self.rtb_correction_type = ct
            return
        self.cb_run_ppk.blockSignals(True)
        self.cb_no_ppk_rtb.blockSignals(True)
        self.cb_run_ppk.setChecked(ct == "ppk")
        self.cb_no_ppk_rtb.setChecked(ct == "rtb")
        self.cb_run_ppk.blockSignals(False)
        self.cb_no_ppk_rtb.blockSignals(False)
        self.rtb_correction_type = ct
    
    def _on_cb_run_ppk(self, state: int) -> None:
        if not self.cb_run_ppk or not self.cb_no_ppk_rtb:
            return
        if state == Qt.Checked:
            self.cb_no_ppk_rtb.blockSignals(True)
            self.cb_no_ppk_rtb.setChecked(False)
            self.cb_no_ppk_rtb.blockSignals(False)
            self.rtb_correction_type = "ppk"
        elif not self.cb_no_ppk_rtb.isChecked():
            self.cb_run_ppk.blockSignals(True)
            self.cb_run_ppk.setChecked(True)
            self.cb_run_ppk.blockSignals(False)
            self.rtb_correction_type = "ppk"
    
    def _on_cb_no_ppk_rtb(self, state: int) -> None:
        if not self.cb_run_ppk or not self.cb_no_ppk_rtb:
            return
        if state == Qt.Checked:
            self.cb_run_ppk.blockSignals(True)
            self.cb_run_ppk.setChecked(False)
            self.cb_run_ppk.blockSignals(False)
            self.rtb_correction_type = "skip"
        elif not self.cb_run_ppk.isChecked():
            self.cb_run_ppk.blockSignals(True)
            self.cb_run_ppk.setChecked(True)
            self.cb_run_ppk.blockSignals(False)
            self.rtb_correction_type = "ppk"
    
    def _sync_rtb_correction_from_checkboxes(self) -> None:
        if self.cb_run_ppk and self.cb_run_ppk.isChecked():
            self.rtb_correction_type = "ppk"
        elif self.cb_no_ppk_rtb and self.cb_no_ppk_rtb.isChecked():
            self.rtb_correction_type = "skip"
        else:
            self.rtb_correction_type = "ppk"
    ...
    def _start_processing(self):
        ...
        self._sync_rtb_correction_from_checkboxes()
